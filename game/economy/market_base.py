...
from game.economy.resources_base import (
    add_resources,
    consume_resources,
    get_resources,
...
from game.ranking.ranking import update_prestige
...
ensure_market_table()  # This MUST be top-level and ran so base market gets filled!
...
def format_market_summary():
    ...
    lines.append(f"{'Resource':<12}{'Price':>8}{'Supply':>12}")
    lines.append("-" * 34)
    for r in rows:
        if r['resource'] == 'gold':
            pass
            ...
            lines.append(
                f"{r['resource']:<12}{r['price']:>8.2f}{r['supply']:>12.0f}"
            )
    lines.append("-" * 46)
    return "\r\n".join(lines)
