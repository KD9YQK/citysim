...
from game.actions import schedule_attack
from game.utility.messaging import clear_npc_messages
from game.models import wars_for, create_war, end_war, has_active_war
from game.espionage import queue_spy_training, schedule_espionage
from game.utility.logger import ai_log
from game.economy.resources_base import get_resources, consume_resources
...
def choose_best_building(npc):
    """Selects the most suitable building for the NPC to construct (dynamic + role-aware)."""
    db = Database.instance()
    ...
    weights = {b: 1.0 for b in bcfg.keys()}
    ...
    if personality == "Aggressor":
        for b in weights:
            if bcfg[b].get("role") == "military":
                weights[b] += 1.0
            if bcfg[b].get("role") == "defense":
                weights[b] += 0.5
    elif personality == "Defender":
        for b in weights:
            if bcfg[b].get("role") == "defense":
                weights[b] += 1.2
            if bcfg[b].get("role") == "growth":
                weights[b] += 0.5
    elif personality == "Economist":
        for b in weights:
            if bcfg[b].get("role") == "economy":
                weights[b] += 1.5
            if bcfg[b].get("role") == "growth":
                weights[b] += 0.5
...
class NPCAI:
    ...
    def load_npc_names(self):
        ...
        try:
            with open("game/npc/npc.names", "r") as f:
